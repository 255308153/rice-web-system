    ...
    <div class="message-container">
      <div class="side-panels">
        <section class="panel contacts">
          ...
          <div class="panel-filters">
            ...
            <input
              v-model.trim="contactKeyword"
              placeholder="搜索联系人"
              @keyup.enter="loadContacts"
              @input="onContactKeywordInput"
            />
            <button class="btn-contact-search" @click="loadContacts" :disabled="contactLoading">搜索</button>
          </div>
          <div class="contact-list">
            <div class="contact-item" v-for="contact in contacts" :key="contact.id" @click="startChat(contact)">
              <img
                v-if="contact.avatar"
                class="mini-avatar avatar-image"
                :src="resolveImageUrl(contact.avatar)"
                alt="联系人头像"
              />
              <div v-else class="mini-avatar">{{ getAvatarText(contact.username, contact.id) }}</div>
          ...
          <div class="conversation-list">
            ...
            >
              <img
                ...
                class="mini-avatar conv-avatar avatar-image"
                :src="resolveImageUrl(conv.peerAvatar)"
                alt="会话头像"
              />
              <div v-else class="mini-avatar conv-avatar">{{ getAvatarText(conv.peerName, conv.peerId) }}</div>
              <div class="conv-main">
                <div class="conv-top">
      ...
      <div class="message-area">
        <div class="chat-title" v-if="currentConvId">
          <img
            v-if="currentReceiverAvatar"
            class="chat-title-avatar"
            :src="resolveImageUrl(currentReceiverAvatar)"
            alt="聊天对象头像"
          />
          <div v-else class="chat-title-avatar chat-title-avatar-fallback">
            {{ getAvatarText(currentReceiverName, currentReceiverId) }}
          </div>
          ...
          >
            <div class="msg-item" v-for="msg in messages" :key="msg.id" :class="isSelfMessage(msg) ? 'msg-self' : 'msg-other'">
              <img
                ...
                class="msg-avatar avatar-image"
                :src="resolveImageUrl(msg.senderAvatar)"
                alt="消息头像"
              />
              <div v-else class="msg-avatar">
                {{ getAvatarText(msg.senderName, msg.senderId) }}
              </div>
        ...
        <div v-else class="empty-state">
          <div class="empty-message">请选择联系人或会话开始聊天</div>
        </div>
      </div>
    </div>
  </div>
...
const currentReceiverAvatar = ref('')
...
const loadContacts = async () => {
  ...
  try {
    ...
    if (contactRole.value) {
      params.role = contactRole.value
    }
    const keyword = String(contactKeyword.value || '').trim()
    if (keyword) {
      params.keyword = keyword
    }

    const res = await request.get('/messages/contacts', { params })
    if (res.code === 200) {
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      if (currentConvId.value) {
        const current = list.find(item => String(item.id) === String(currentConvId.value))
        if (current) {
          currentReceiverId.value = current.peerId || (String(current.user1Id) === String(userId.value) ? current.user2Id : current.user1Id)
          currentReceiverName.value = current.peerName || ''
          currentReceiverAvatar.value = current.peerAvatar || ''
        }
      }
    }
  } catch (e) {
...
const selectConv = async (conv) => {
  ...
  currentReceiverAvatar.value = conv.peerAvatar || ''
  shouldAutoScroll.value = true
